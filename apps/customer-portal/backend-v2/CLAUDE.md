...
`CSM_MIGRATION_DIRECT_ONBOARDING_ENABLED` (`ContactHandler.syncMembership`) — after `CreateProjectContact`, `RemoveProjectContact` or `UpdateProjectContactRole` has written Salesforce, calls entity-service `POST /project-contacts/{sfId}/sync` with the membership's Salesforce id, so the CSM database — and, for a new invitation, the Asgardeo account and the invitation email that follow from it — are done inside the admin's own request. Without it the same work still happens, but only once the Salesforce change event has travelled publisher → Service Bus → subscriber → entity-service.

It is **best-effort, and that is not a compromise**: by the time it runs the Salesforce write has already succeeded, and its change event reaches entity-service by the old path regardless. A failure therefore costs a few seconds of freshness, not the onboarding, and failing the admin's request over it would tell them their change did not happen when it did. entity-service also only registers that route when its membership ingest is on, and restricts it to internal callers, so a 404 or 403 is the normal answer from a deployment that has not cut over.

Duplicate suppression is not this service's problem: entity-service's ingest ignores a Salesforce version it has already seen, and csm-notification-service checks the onboarding-step ledger before sending an invitation.
