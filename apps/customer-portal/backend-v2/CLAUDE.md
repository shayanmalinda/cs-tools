...
- **The portal authorizes every write itself.** entity-service only checks that the caller is an allow-listed internal client, so `requireProjectAdmin` refuses with `403` unless the caller is an active CS admin on the project's contact list. That list is read from Salesforce by the project's Salesforce Id, so the check does not depend on how entity-service scopes the portal's reads. Never add a write on this path without it.
