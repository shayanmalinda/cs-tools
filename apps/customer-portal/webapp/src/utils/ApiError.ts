...
/**
 * Throws an ApiError for non-OK responses while preserving backend message.
 *
 * @param {Response} response - Fetch response to inspect.
 * @param {string} fallbackMessage - Fallback message when body has no message.
 * @returns {Promise<void>} Resolves for OK responses; throws otherwise.
 */
export async function throwIfApiNotOk(
  response: Response,
  ...
  try {
    ...
    if (
      ...
      parsedMessage = (errorBody as { message: string }).message;
    }
  } catch {
  ...
  throw new ApiError(
    response.status,
    response.statusText,
    parsedMessage ?? `${fallbackMessage}: ${response.status} ${response.statusText}`,
  );
}
